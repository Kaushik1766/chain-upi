...
type Wallet struct {
	...
	Chain      string    `gorm:"not null;type:varchar(10);primaryKey"`
}

func (w *Wallet) ToString() string {
	s := fmt.Sprintf("UserUID: %s, Name: %s, Address: %s, Chain: %s, IsPrimary: %t",
		w.UserUID.String(), w.User.Name, w.Address, w.Chain, w.IsPrimary)
	return s
}
